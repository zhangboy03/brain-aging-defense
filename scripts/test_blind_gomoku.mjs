...
function altMoves(s, blackCells) {  // 黑方按给定坐标走，白方依次垫远处
  ...
  for (const [x, y] of blackCells) {
    ...
    out.push({ team: teamOf(s, 'w'), x: wi++, y: G.SIZE - 1, color: 'blue' });
  }
...
test('T8 判定只看真实色不看表面色（白方用 purple 表面子连五）', () => {
  const s = freshGame(); const W = teamOf(s, 'w'), B = teamOf(s, 'b');
  const seq = [];
  // 黑方垫在 14 行且间隔落子（x=0,2,4,6,8——绝不能连续 5 格，否则黑先连五），
  // 白方在第 5 行连五，全用 'purple' 表面色
  for (let i = 0; i < 5; i++) {
    seq.push({ team: B, x: i * 2, y: G.SIZE - 1, color: 'green' });
    seq.push({ team: W, x: 3 + i, y: 5, color: 'purple' });
  }
  play(s, seq);
...
test('E3 越界/非整数被拒', () => {
  ...
  for (const [x, y] of [[G.SIZE, 0], [-1, 0], [0, G.SIZE], [2.5, 3], [NaN, 1]]) {
...
test('E3b 黑白不再是可选表面色', () => {
  const s = freshGame(); const v0 = s.v; const B = teamOf(s, 'b');
  assert.equal(G.applyMove(s, { gameId: s.gameId, team: B, x: 7, y: 7, color: 'black' }), false);
  assert.equal(G.applyMove(s, { gameId: s.gameId, team: B, x: 7, y: 7, color: 'white' }), false);
  assert.deepEqual(G.COLORS, ['red', 'yellow', 'blue', 'green', 'pink', 'purple']);
  assert.equal(s.v, v0);
});
...
test('E7 361 手下满无连五 → draw', () => {
  const s = freshGame();
  // 用两行一换的 4 列循环模式保证 (18,18) 落子处任何方向不足 5 连；
  // 若模式意外连五，下方显式 fail 提示换模式（断言守护，不靠人脑验证）。
  const pat = (x, y) => ((x + Math.floor(y / 2)) % 4) < 2 ? 'b' : 'w';
  const last = G.SIZE - 1;
  for (let y = 0; y < G.SIZE; y++) for (let x = 0; x < G.SIZE; x++) {
    if (x === last && y === last) continue;
    s.board[x][y] = { s: pat(x, y), c: 'red', n: 0 };
  }
  s.moveCount = G.SIZE * G.SIZE - 1;
  s.turn = pat(last, last);
  const team = teamOf(s, s.turn);
  assert.equal(G.applyMove(s, { gameId: s.gameId, team, x: last, y: last, color: 'red' }), true);
  if (s.winner !== 'draw') {
...
test('E9 第 361 手同时连五 → 算胜不算平', () => {
  const s = freshGame();
  for (let y = 0; y < G.SIZE; y++) for (let x = 0; x < G.SIZE; x++) s.board[x][y] = { s: 'w', c: 'red', n: 0 };
  // 腾出一行五连位给黑方：(0..4, 7)，其中 (4,7) 留空为最后一手
  for (let x = 0; x < 4; x++) s.board[x][7] = { s: 'b', c: 'red', n: 0 };
  s.board[4][7] = null;
  s.moveCount = G.SIZE * G.SIZE - 1; s.turn = 'b';
